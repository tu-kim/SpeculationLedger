...
파생 관찰(코너 케이스 검증에서 확인): freq×p̂ 랭킹은 자가치유적이다 — 기각이 쌓이면
교정 토큰이 빈도·p̂로 top을 탈환하므로, §3.2의 reject-dominant splice는 '증거 충돌'
(target이 계속 지지하는데 검증은 계속 기각)의 좁은 창에서만 발동하는 백스톱이다.
단일 안정 교정 하에서는 top-이-기각-우세 상태가 구성적으로 불가능하다
(acc(correction)=rej(X)>acc(X) → 빈도가 먼저 역전). tests/test_corner_cases.py의
splice 2종 테스트가 이 동역학을 고정한다.

### A-9. 감사 조율 (2026-07-12, core 전면 감사 후) — RESOLVED

계약 §3.1과 구현의 조율 3건. 크래시급 결함은 없었고, 아래는 계약-구현 정합 문제다.

- **coverage_target 배선 (Finding A):** `StoreParams.coverage_target`가 정의만 되고
  적응-k 게이트는 `0.9`를 하드코딩했었다. 파라미터를 `_ensure`/`merge_topk`까지
  배선(기본값 0.9 → byte-동일 보존). tests/test_audit_reconciliation.py가
  배선 여부(cov 0.0 vs 1.0에서 k_cap 분기)를 회귀 방어.

- **단일 λ / fold_key 인라인 사본 (Finding C):** G4 사전 최적화로 blend의 λ 계산과
  store 핫루프의 fold_key를 인라인 전개했다. 계약 §3.1의 "단일 보간 λ"는 *의미*의
  단일성을 뜻하며, 성능상 인라인 사본은 허용하되 **동치를 테스트로 강제**한다:
  test_inline_fold_key_equals_spec_function_exhaustively(2000 케이스),
  test_inline_lambda_weight_equals_lam(500 케이스). 주석 기반 보증 → 테스트 강제로 승격.
  (원본 lam()/fold_key()는 스펙 정의로 유지 — 한쪽 수정 시 테스트가 즉시 실패.)

- **greedy correction = p̂ argmax (Finding D):** 계약 §3.1/§3.4의 correction 규칙을
  `Posterior.correction()`로 명시(= cands 중 p_hat 최대, 동률은 tok). 기존
  `argmax()`는 *제안* 랭킹(복합점수 top)이며 이 둘은 다봉 문맥에서 갈린다 —
  의도된 분리다(A-8). proposer의 splice 경로도 correction()을 쓰도록 통일
  (기존 수식과 동일 → byte-동일). **계약 개정 제안:** §3.1의 "greedy의 correction =
  p̂ argmax"는 Posterior.correction() 접근자로 실현되며, proposal 랭킹(argmax)과
  구분됨을 명문화할 것.

- **미해결로 남긴 것 (Finding B, 계약 개정 후보):** HotEntry가 epoch 검증을 위해
  `scope_id`+`dom`을 엔트리별 보유 → 문서화된 48–64B 레이아웃 초과. Python 참조
  구현에서는 무해하나, **네이티브 포팅(G4) 시 레이아웃에 이 둘을 반영하거나 epoch
  도메인 결속을 엔트리 밖으로 빼는 결정**이 필요하다. Phase 1 진입 시 확정.
